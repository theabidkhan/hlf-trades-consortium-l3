...
var logger = log4js.getLogger('BanksTradeConsortium');
...
MongoClient.connect("mongodb://localhost:27017/dbBanksTradeConsortium", { useNewUrlParser: true, useUnifiedTopology: true  });

var db=MongoClient.connection; 
db.on('error', console.log.bind(console, "Mongo DB Connection Error")); 
db.once('open', function(callback){ 
    console.log("Connection Succeeded for dbBanksTradeConsortium"); 
}) 
...
{
	//Generating UserID with the combination of bankName and a 6 digit random number
	var key=Math.random().toString().replace('0.', '');
	key=key.substring(0,6);
	
	var UserID = req.body.BankName+key; //userID Format : IDFC100001
	...
	//inserting the userData into users 
	db.collection('users').insertOne(data,function(err, userData)
	{ 
		if (err) throw err; 
		console.log("User Registered Successfully"); 
		console.log(userData);
		return res.json({"success":"true","message":"User registered successfully","UserID":UserID});          
    });  
}) 
...
{ 
	...
	//Validating UserID and Password from DB
	db.collection('users').find({"UserID":UserID, "Password":Password}).toArray(function(err, userData) 
	{
		if (err) throw err;
...
app.post('/channels/:channelName/chaincodes', async function(req, res) {
	...
	//Added by Ramesh on 16 Jan 
	//To upgrade and instantiate the Chain Code 
	var instantiateType = req.body.instantiateType;
	...
	let message = await instantiate.instantiateChaincode(peers, channelName, chaincodeName, chaincodeVersion, chaincodeType, fcn, args, req.username, req.orgname, instantiateType);
	res.send(message);
});
