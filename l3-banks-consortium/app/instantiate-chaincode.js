...
const collectionsConfigPath = path.resolve(__dirname, '../collections_config.json');
console.log('Collection Config Path ----> '+collectionsConfigPath);
const instantiateChaincode = async function(peers, channelName, chaincodeName, chaincodeVersion, functionName, chaincodeType, args, username, org_name, instantiateType) {
	logger.debug('\n\n============ Instantiate chaincode on channel ' + channelName +
		' ============\n');
	let error_message = null;
	let client = null;
	let channel = null;
	let data = null;//Get USer Data
	try {
		...
		var request = {};
            ...
            request = {
                ...
                'endorsement-policy': {
                        identities: [
                        { role: { name: 'member', mspId: 'idfcMSP' }},
						{ role: { name: 'member', mspId: 'hsbcMSP' }},
						{ role: { name: 'member', mspId: 'idbiMSP' }}
                        ],							
                        policy: {
			...
			console.log('Instantiate Request Parameters: ');
			console.log(request);			
		...
		// let results = await channel.sendUpgradeProposal(request, 120000); //instantiate takes much longer
		// console.log(results);
		console.log('-------------------------------');
		...
		if (all_good) {
			logger.info(util.format(
				...
				data = proposalResponses[0].response.payload.data//Get User Data
